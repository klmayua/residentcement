...
const tierConfig: Record<string, { color: string; label: string }> = {
 ...
 silver: { color:"bg-cement-100 text-cement-800", label:"Silver"},
 gold: { color:"bg-yellow-100 text-yellow-800", label:"Gold"},
 platinum: { color:"bg-blue-100 text-blue-800", label:"Platinum"},
};

const statusConfig: Record<string, { variant: string; label: string }> = {
 active: { variant:"bg-green-100 text-green-800", label:"Active"},
 inactive: { variant:"bg-cement-100 text-cement-800", label:"Inactive"},
 suspended: { variant:"bg-red-100 text-red-800", label:"Suspended"},
};
...
export default function CustomersPage() {
 ...
 <p className="text-cement-500">Manage your customer relationships</p>
 </div>
 ...
 <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-cement-500">Contact</th>
 <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-cement-500">Tier</th>
 <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-cement-500">Credit Limit</th>
 <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-cement-500">Balance</th>
 <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-cement-500">Orders</th>
 ...
 <tr key={customer.id} className="hover:bg-cement-50">
 <td className="whitespace-nowrap px-6 py-4">
 <div>
 <div className="font-medium text-cement-900">{customer.name}</div>
 <div className="text-sm text-cement-500">{customer.id}</div>
 </div>
 </td>
 <td className="whitespace-nowrap px-6 py-4">
 <div className="flex flex-col space-y-1 text-sm">
 <div className="flex items-center text-cement-600">
 ...
 <td className="whitespace-nowrap px-6 py-4">
 <button className="rounded p-1 hover:bg-cement-100">
 <MoreVertical className="h-4 w-4 text-cement-500"/>
 </button>
 </td>
 </tr>
 ))}
