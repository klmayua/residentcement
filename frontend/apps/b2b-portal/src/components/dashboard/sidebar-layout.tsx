...
import {
  LayoutDashboard, Package, ShoppingCart, Receipt, BarChart3, Settings,
  LogOut, Bell, Menu, X
} from 'lucide-react';
...
function SidebarLayoutContent({ children, title, subtitle }: {
  ...
  const company = user?.companyName ?? 'Enterprise Partner';
      ...
      <aside className={`w-64 bg-[#161311] border-r border-[#292524]/50 flex flex-col h-screen fixed left-0 top-0 z-40 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="px-5 h-16 flex items-center justify-between border-b border-[#292524]/30 gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="font-serif text-base text-[#e5c374] font-bold truncate">
              ResidentCiment
            </div>
            <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-[#4d4540] border-l border-[#292524] pl-3 whitespace-nowrap">B2B Portal</span>
          </div>
          <button
        ...
        <div className="p-4 border-t border-[#292524]/30">
          ...
          <a
            ...
            className="w-full flex items-center gap-2 px-3 py-2 mb-2 text-[10px] font-bold uppercase tracking-widest text-[#57534e] hover:text-[#e5c374] hover:bg-[#1c1917] transition-colors"
          >
          ...
          <button
            onClick={logout}
            className="w-full flex items-center gap-2 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[#57534e] hover:text-[#a8a29e] hover:bg-[#1c1917] transition-colors"
          >
